...
namespace WinformControlLibraryExtension
{
    ...
    public class HRulerExt : Control
    {
        #region 停用事件
        ...
        private float realityWidth = 500f;
        /// <summary>
        /// 内容真实宽度
        /// </summary>
        [Description("内容真实宽度")]
        [DefaultValue(500f)]
        public float RealityWidth
        {
            get
            {
            ...
            set
            {
                if (this.realityWidth == value)
                    return;

                this.realityWidth = value;
                this.Invalidate();
            }
        }

        private float displayWidth = 500f;
        /// <summary>
        /// 内容真实宽度对应在面板显示宽度
        /// </summary>
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            float ruler_extent_ratio = this.RealityWidth / this.DisplayWidth;//真实内容和尺子比例
            ...
            g.DrawLine(master_pen, new Point(this.ClientRectangle.Left, this.ClientRectangle.Bottom - 1), new Point(this.ClientRectangle.Right, this.ClientRectangle.Bottom - 1));

            for (int i = 0; ; i++)//正向尺子
            {
                //主刻度
                float current_main_scale_x = this.MasterStartPoint + i * main_scale_interval_px;

                //次刻度
                for (int k = 0; k < minor_scale_count; k++)
                {
                    float current_minor_scale_x = current_main_scale_x + k * minor_scale_pixel;
                    g.DrawLine(second_pen, new PointF(current_minor_scale_x, this.ClientRectangle.Bottom - this.ClientRectangle.Height / 3), new PointF(current_minor_scale_x, this.ClientRectangle.Bottom));
                }

                g.DrawLine(master_pen, new PointF(current_main_scale_x, this.ClientRectangle.Top), new PointF(current_main_scale_x, this.ClientRectangle.Bottom));
                //主刻度文本
                string current_main_scale_str = ((int)(i * single_main_scale_value)).ToString();
                g.DrawString(current_main_scale_str, this.Font, text_sb, new PointF(current_main_scale_x + 2, this.ClientRectangle.Top));

                if (this.MasterStartPoint + (i + 1) * main_scale_interval_px >= this.ClientRectangle.Right)
                {
                    break;
                }
            }
